...
that compose fails fast when GF_ADMIN_PASSWORD is unset.
...
# Scripts allowed to mention `Authorization: Basic` because they compose the
# header value from environment credentials at runtime rather than embedding
# one. Membership is not a blanket exemption: the sweep below still asserts
# each entry actually interpolates, so a real literal cannot hide here.
RUNTIME_AUTH_ALLOWLIST = {"scripts/test-promtail-host-label.sh"}
...
class TestNoSilentCredentialFallbacks(unittest.TestCase):
    ...
    def test_no_silent_admin_fallback_in_compose(self) -> None:
        ...
        # #613 wires GF_SECURITY_ADMIN_USER to ${GRAFANA_ADMIN_USER:-admin}.
        # The username default is intentional and non-secret; strip it before
        # sweeping for password fallbacks (#318 must stay fail-fast).
        content = content.replace("${GRAFANA_ADMIN_USER:-admin}", "")
    ...
    def test_no_unauthenticated_basic_auth_header_literals(self) -> None:
        ...
        for path in (REPO_ROOT / "scripts").rglob("*"):
            ...
            try:
                ...
                continue
            if rel in RUNTIME_AUTH_ALLOWLIST:
                self.assertIn(
                    "Authorization: Basic ${",
                    text,
                    f"{rel} is allowlisted for runtime Basic auth but does not "
                    f"interpolate its credentials",
                )
                continue
...
class TestComposeFailFastBehavior(unittest.TestCase):
    ...
    def test_compose_failfast_when_grafana_password_unset(self) -> None:
        """compose config exits non-zero and names GF_ADMIN_PASSWORD."""
        result = self._compose_config()
        self.assertNotEqual(
            result.returncode,
            0,
            "docker compose config succeeded with GF_ADMIN_PASSWORD unset; "
            "the silent :-admin fallback appears to be back",
        )
        self.assertIn("GF_ADMIN_PASSWORD", result.stderr)
